...
Use a passphrase that suits your paranoia, but *never* use the same passphrase for multiple files/messages (!). Doing so would render any encryption almost completely useless, as it allows for statistic analysis on the encrypted files.
